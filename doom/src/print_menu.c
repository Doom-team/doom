...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/26 20:41:59 by ahusk             #+#    #+#             */
/*   Updated: 2021/02/03 21:17:27 by wendell          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (button->bytes_texture[pos->x2 * 4 + 3 +
		pos->y2 * button->texture->pitch] != 0)
	{
		sdl->bytes[pos->x1 * 4 + 0 + pos->y1 * sdl->pitch] =
			button->bytes_texture[pos->x2 * 4 + 0 +
				pos->y2 * button->texture->pitch];
		sdl->bytes[pos->x1 * 4 + 1 + pos->y1 * sdl->pitch] =
			button->bytes_texture[pos->x2 * 4 + 1 +
				pos->y2 * button->texture->pitch];
		sdl->bytes[pos->x1 * 4 + 2 + pos->y1 * sdl->pitch] =
			button->bytes_texture[pos->x2 * 4 + 2 +
				pos->y2 * button->texture->pitch];
		sdl->bytes[pos->x1 * 4 + 3 + pos->y1 * sdl->pitch] =
			button->bytes_texture[pos->x2 * 4 + 3 +
				pos->y2 * button->texture->pitch];
	}
}
...
void	draw_button(t_sdl *sdl, t_button *button)
{
	...
	while (++pos.x1 < button->pos.x2)
	{
		...
		pos.x2 = ((pos.x1 - button->pos.x1) * button->texture->w) /
			(button->pos.x2 - button->pos.x1);
		while (++pos.y1 < button->pos.y2)
		{
			pos.y2 = ((pos.y1 - button->pos.y1) * button->texture->h) /
				(button->pos.y2 - button->pos.y1);
